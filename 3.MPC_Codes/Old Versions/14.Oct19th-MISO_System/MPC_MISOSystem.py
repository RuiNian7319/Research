...
from MISO_CasADI import MISOSystem
...
def simulation():
    ...
    # Plant Model
    model_plant = MISOSystem(nsim=30, x0=np.array([3]), u0=np.array([3, 6]), xs=np.array([4.5]), us=np.array([5.5, 8]),
                             step_size=0.2, control=False, q_cost=1, r_cost=0.5, random_seed=1)

    # Build Controller Model
    model_control = MISOSystem(nsim=model_plant.Nsim, x0=np.array([3, 0]), u0=np.array([3, 6]), xs=np.array([4.5, 0]),
                               us=np.array([5.5, 8]), step_size=0.2, control=True, q_cost=1, r_cost=0.5, random_seed=1)
