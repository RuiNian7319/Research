...
class ModelPredictiveControl:
    ...
    def get_mpc_controller(self, ode_casadi, delta, x0, random_guess=False, c=5, verbosity=0, upper_bound=1.2,
                           lower_bound=0.8):
    ...
    def solve_mpc(self, model_states, model_sp, mpc_controller, sim_time, p):
        ...
        for i in range(self.eval_time):
            model_input.append(np.squeeze(mpc_controller.var["u", i]))

        # Find the predicted x value from the controller
        x_predicted = []

        for i in range(1, self.eval_time + 1):
            x_predicted.append(np.squeeze(mpc_controller.var["x", i]))
