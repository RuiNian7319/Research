...
class MimoCstr:
    ...
    def __init__(self, nsim, delta=1, nx=3, nu=2, nt=10, f0=0.1, t0=350, c0=1, r=0.219, k0=7.2e10, er_ratio=8750,
                 ...
                 x0=np.array([1, 310, 0.659]), u0=np.array([300, 0.1]), control=False, q_cost=0.1, r_cost=0.1):
    ...
    def reward_function(self, t, action):
        ...
        if self.xs[1] * 0.99 < self.x[t, 1] < self.xs[1] * 1.01:
            rewards = rewards + 15 - (abs(self.x[t, 1] - self.xs[1]) * 15)
        else:
            rewards = rewards - abs(self.x[t, 1] - self.xs[1])

        # Action penalty
        rewards = rewards - ((action * 0.1) ** 2)
        # rewards = rewards - abs(action - self.us[0])
        ...
        # r = self.r_cost * np.eye(self.Nu)
        #
        # rewards = -(dx @ q @ dx.T + du @ r @ du.T) + 0.2
        #
        # Clipping rewards to improve stability
        rewards = max(-100, rewards)
...
if __name__ == "__main__":
    ...
    cstr = MimoCstr(nsim=200)

    for time in range(1, cstr.Nsim + 1):

        if time % 21 == 0:
            actions = -10  # np.random.uniform(-2, 2)
        elif time % 23 == 0:
            actions = -15
        else:
            actions = 0
