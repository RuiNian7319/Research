    ...
    def priority_sample_batch(self, batch_size, type='rank'):
        ...
        # If rank priority replay is selected
            ...
            for k in range(batch_size):
                index = np.random.choice(np.linspace(1, self.count, num=self.count), size=1, p=prob_list)
                batch.append(self.buffer.heap[int(index[0])])

                # Not 100% accurate because the pop_mid messes with index, but close enough
                self.buffer.pop_mid(int(index[0]))

                prob_list.remove(prob_list[int(index[0]) - 1])
                prob_list = list(np.divide(prob_list, np.sum(prob_list)))
