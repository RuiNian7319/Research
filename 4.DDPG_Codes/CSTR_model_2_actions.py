...
class MimoCstr:
    ...
    def __init__(self, nsim, delta=1, nx=3, nu=2, nt=10, f0=0.1, t0=350, c0=1, r=0.219, k0=7.2e10, er_ratio=8750,
        ...
        self.action_space = Box(low=np.array([-1.5, -0.001]), high=np.array([1.5, 0.001]))
    ...
    def reward_function(self, t, action):
        ...
        if self.xs[1] * 0.998 < self.x[t, 1] < self.xs[1] * 1.002:
            rewards = rewards + 15 - abs(self.x[t, 1] - self.xs[1])
        else:
        ...
        if self.xs[2] * 0.99 < self.x[t, 2] < self.xs[2] * 1.01:
            ...
            rewards = rewards - abs(self.x[t, 2] - self.xs[2]) * 5

        # Action penalty
        rewards = rewards - (0.01 * action**2)

        # Clipping rewards to improve stability
        # rewards = max(-50, rewards)
