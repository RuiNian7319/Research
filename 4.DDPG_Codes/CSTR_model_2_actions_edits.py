...
class MimoCstr:
    ...
    def __init__(self, nsim, delta=1, nx=3, nu=2, nt=10, f0=0.1, t0=350, c0=1, r=0.219, k0=7.2e10, er_ratio=8750,
                 u=54.94, rho=1000, cp=0.239, dh=-5e4, xs=np.array([0.894, 309.8, 2]), us=np.array([300, 0.1]),
                 x0=np.array([1, 310, 2]), control=False):
        ...
        self.action_space = Box(low=np.array([-2.5, -0.00]), high=np.array([2.5, 0.00]))
    ...
    def reset(self, random_init=False):
        ...
        if random_init is True:
            self.x[0, 1] = self.x[0, 1] + np.random.uniform(-5, 5)
        else:
            pass
    ...
    def step(self, action, t):
        ...
        self.u[t, 0] = self.u[t - 1, 0] + action[0]
        self.u[t, 1] = self.u[t - 1, 1] + action[1]
        ...
        # Temperature approaching absolute 0
        elif self.x[t, 1] < 10:
            done = True
        # Tank approaching empty
        elif self.x[t, 2] < 1:
            done = True
        else:
            done = False
    ...
    def reward_function(self, t, action):
        # Handle multiple action case
        action = np.sum(action)

        rewards = 0

        # Setpoint tracking errors
        # if self.xs[0] * 0.99 < self.x[t, 0] < self.xs[0] * 1.01:
        #     rewards = rewards + 5
        # else:
        #     rewards = rewards - abs(self.x[t, 0] - self.xs[0]) * 5
        ...
        if self.xs[2] * 0.99 < self.x[t, 2] < self.xs[2] * 1.01:
            rewards = rewards + 15 - (abs(self.x[t, 2] - self.xs[2]) * 600)
        else:
            rewards = rewards - (abs(self.x[t, 2] - self.xs[2]) * 15)

        # Action penalty
        rewards = rewards - (0.1 * action**2)

        # Clipping rewards to improve stability
        rewards = max(-35, rewards)
...
if __name__ == "__main__":
    ...
    # env = MimoCstr(nsim=50, nx=6, xs=np.array([0.878, 324.5, 0.659, 0, 0, 0]),
    #                 x0=np.array([1, 310, 0.659, 0, 0, 0]), control=True)

    env = MimoCstr(nsim=250)

    for time in range(1, env.Nsim + 1):

        if time == 50:
            actions = np.array([0, 0.00])  # np.random.uniform(-2, 2)
        elif time == 51:
            actions = np.array([0, 0.00])
        else:
            actions = np.array([0, 0.0])

        State, Reward, Done, _ = env.step(actions, time)

        # env.u[time, :] = env.us
        # str.x[time, :] = env.next_state(env.x[time - 1, :], env.u[time - 1, :])
        #
        # if time == 30:
        #     env.disturbance()
        #
