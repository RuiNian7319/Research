...
class ModelPredictiveControl:
    ...
    def get_mpc_controller(self, ode_casadi, delta, x0, random_guess=False, c=3, verbosity=0, upper_bound=1.2,
                           lower_bound=0.8):
        ...
        contargs = dict(
            ...
            #     "x": upper_bound * np.concatenate([self.ss_states[0:int(self.Nx * 0.5)], [np.inf, np.inf]])},
            # lb={"u": lower_bound * self.ss_inputs,
            #     "x": lower_bound * np.concatenate([self.ss_states[0:int(self.Nx * 0.5)], [-np.inf, -np.inf]])},
            guess=guess
        )
