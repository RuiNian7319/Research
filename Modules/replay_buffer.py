...
class ReplayBuffer(object):
    ...
    def add(self, s, a, r, t, s2):
        ...
        # List type so we can change it later
    ...
    """
    Finds the real mean and standard deviation of the state space.  First, fill the buffer with sufficient memories,
    then based on those memories, calculate the mean and standard deviation.
    """

    def normalize(self):

        # Ensure input states and output states are of the same size
        assert(len(self.buffer[0][0]) == len(self.buffer[0][4]))

        # Initialization of the running sum
        running_sum = np.empty((0, len(self.buffer[0][0])))

        # Stack all the states on top of each other to do analysis
        for memory in range(self.size()):
            running_sum = np.vstack((running_sum, self.buffer[memory][0]))
            running_sum = np.vstack((running_sum, self.buffer[memory][4]))

        # Add 0.01 to avoid division by zero
        mean = np.mean(running_sum, axis=0)
        st_dev = np.std(running_sum, axis=0) + 0.01

        # Normalize the existing data in the replay memory
        for i in range(self.size()):
            self.buffer[i][0] -= mean
            self.buffer[i][0] /= st_dev

            self.buffer[i][4] -= mean
            self.buffer[i][4] /= st_dev
    ...
    def load_replay(self, path):
        ...
        self.buffer = pickle.load(pickle_in)
