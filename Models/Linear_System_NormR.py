...
class LinearSystem:
    ...
    def __init__(self, nsim, model_type='SISO', x0=np.array([0.5]), u0=np.array([1]), xs=np.array([5]),
                 us=np.array([10]), step_size=0.1, control=False, q_cost=1, r_cost=0.5, s_cost=0.1, random_seed=1):
    ...
    def reward_function(self, step, obj_function='RL', delta_u="none"):
        ...
        if obj_function == "RL":
            ...
            if self.model_type == "SISO":
                ...
                if delta_u == "l2":
                    reward = - 0.375 * abs(x * self.Q * x + u * self.R * u + du * self.S * du) + 1

                elif delta_u == "l1":
                    reward = - 0.375 * abs(x * self.Q * x + u * self.R * u + abs(du * self.S)) + 1

                else:
                    reward = - 0.375 * abs(x * self.Q * x + u * self.R * u) + 1
                ...
                if delta_u == "l2":
                    reward = - 0.77 * (abs(x.T @ self.Q @ x + u.T @ self.R @ u + du.T @ self.S @ du)) + 1

                elif delta_u == "l1":
                    reward = - 0.77 * (abs(x.T @ self.Q @ x + u.T @ self.R @ u + sum(abs(du.T @ self.S)))) + 1

                else:
                    reward = - 0.77 * (abs(x.T @ self.Q @ x + u.T @ self.R @ u)) + 1
